...
void main()
{
	...
	{
		outColor += CalculateDirect(light_.mDirects[i], v_out_.mNormal, cameraNormal, v_out_.mUV);
	}

	for (int i = 0; i != light_.mPointNum; ++i)
	{
		outColor += CalculatePoint(light_.mPoints[i], v_out_.mNormal, cameraNormal, v_out_.mUV);
	}

	for (int i = 0; i != light_.mSpotNum; ++i)
	{
		outColor += CalculateSpot(light_.mSpots[i], v_out_.mNormal, cameraNormal, v_out_.mUV);
	}
